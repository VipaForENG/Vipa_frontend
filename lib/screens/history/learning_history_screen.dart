...
class _LearningHistoryView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // [로직] 당겨서 새로고침(RefreshIndicator) 유지
  ...
  Widget _buildBody(BuildContext context, LearningHistoryProvider provider) {
    ...
    return ListView(
      ...
      children: [
        ...
        // [UI & 로직] 팀원의 cardContainer 디자인 + 리더님의 실전회화 데이터
        cardContainer(
        ...
        const SizedBox(height: 15), // 팀원 UI의 카드 간격 적용
        // [UI & 로직] 팀원의 cardContainer 디자인 + 리더님의 오늘의 어휘 데이터
        cardContainer(
          child: _HistorySection(
            title: '오늘의 어휘',
...
// --- 이 아래는 리더님의 main 브랜치 로직 그대로 유지 (UI 충돌 없음) ---
