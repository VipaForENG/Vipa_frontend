...
class _MyPageScreenState extends State<MyPageScreen> {
  ...
  Future<void> _loadUserInfo({bool forceRefresh = false}) async {
    ...
    final cachedLocalImagePath = (savedLocalImagePath != null && savedLocalImagePath.isNotEmpty)
        ? savedLocalImagePath
        : (cachedData is Map ? cachedData['local_profile_image_path']?.toString() : null);
  ...
  Widget build(BuildContext context) {
    // [UI] 팀원의 AppColors.background 배경색 적용 및 main의 RefreshIndicator 결합
    return Scaffold(
          ...
          child: Column(
            children: [
              ...
              cardContainer(
                child: Column(
                  children: [
                    // [로직] main 브랜치의 isSocialUser 검증 활용
                    if (!isSocialUser)
                      _buildMenuItem(
  ...
  // [UI & 로직] 팀원의 UI 레이아웃과 main의 _buildProfileAvatar 로직 결합
  ...
  // [로직] main의 이미지 렌더링 로직 유지 (파일 우선 확인 후 네트워크 이미지 폴백)
  Widget _buildProfileAvatar({required double radius, String? localPath, String? imageUrl}) {
    ...
    if (localPath != null && File(localPath).existsSync()) {
      ...
      image = Image.network(imageUrl, fit: BoxFit.cover, errorBuilder: (_, __, ___) => fallback);
    }
  ...
  Widget _buildMenuItem({
    ...
    return ListTile(
      ...
      title: Text(
        title,
        style: TextStyle(fontSize: 15, fontWeight: FontWeight.w500, color: isDanger ? Colors.redAccent : const Color(0xFF2D3436)),
      ),
